...
void Goods :: Change(){
	...
	for( ; ; ){
		...
		cout<<"4. Change COUNT"<<endl;
		cout<<"5. Exit changes"<<endl;
		...
		switch(key){
			case 1: Read("Enter ID: ", id); break; 
			case 2: cout<<"Enter NAME: "<<endl;
					cin>>name; break;
			case 3: Read("Enter PRICE: ", price); break;
			case 4: Read("Enter COUNT: ", count); break;
			case 5: sum = count * price; 
					Show();
					exit(1);
...
Recipe :: Recipe()
	{
		_Recipe = new unsigned char[1];
		_Recipe[0] = '\0';
	}

unsigned char Recipe :: operator[](int j) {
		return _Recipe[j];
}
